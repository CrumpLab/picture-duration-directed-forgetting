...
The main effect of encoding duration was not significant, `r Accuracy$apa_print$full_result$encoding_stimulus_time`. Proportion correct was similar across the 500 ms (`r qprint_mean_sem(Accuracy$means,"encoding_stimulus_time","500",c("mean_correct","sem_correct")) `), 1000 ms (`r qprint_mean_sem(Accuracy$means,"encoding_stimulus_time","1000",c("mean_correct","sem_correct")) `), and 2000 ms (`r qprint_mean_sem(Accuracy$means,"encoding_stimulus_time","2000",c("mean_correct","sem_correct")) `) stimulus durations.
...
The main question of interest was whether directing forgetting would vary across the encoding duration times. The interaction between encoding instruction and encoding duration was, `r Accuracy$apa_print$full_result$encoding_stimulus_time_encoding_instruction`. 

Paired sample t-tests were used to assess the directed forgetting effect at each encoding duration. The directed forgetting effect is taken as the difference between proportion correct for remember minus forget items. At 500 ms, the directed forgetting effect was reversed and not significant, `r Accuracy$simple$DF_500$full_result`. At 1000ms, the directed forgetting effect was reversed and not significant, `r Accuracy$simple$DF_1000$full_result`. And, at 2000 ms, the directed forgetting effect was again not detected, `r Accuracy$simple$DF_2000$full_result`.

The encoding duration by lure type interaction was, `r Accuracy$apa_print$full_result$encoding_stimulus_time_test_condition`. In the 500 ms condition, proportion correct was higher for novel than exemplar lures, `r Accuracy$simple$test_500$full_result`. The advantage for novel over exemplar items was smaller in the 1000 ms condition, `r Accuracy$simple$test_1000$full_result`, and 2000 ms condition `r Accuracy$simple$test_2000$full_result`.
...
The main effect of encoding instruction was significant, `r RT$apa_print$full_result$encoding_instruction`. Mean reaction times were faster for choosing remember cued items (`r qprint_mean_sem(RT$means,"encoding_instruction","R",c("mean_rt","sem_rt")) `) than forget cued items (`r qprint_mean_sem(RT$means,"encoding_instruction","F",c("mean_rt","sem_rt")) `).
