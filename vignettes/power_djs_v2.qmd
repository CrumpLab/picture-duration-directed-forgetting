...
subjects <- function(num_subjects) {
  ...
  s <- s %>%
    add_column(subject = rep(1:num_subjects, each = 4))
...
test <- function(subjects) {
  anova <- aov(mean_correct ~ test_item_type * cue_type + Error(subject/(test_item_type * cue_type)), subjects)
  
  summary <- summary(anova)
  return(summary$`Error: subject:cue_type`[[1]]$`Pr(>F)`[1])
...
for(i in num_subjects) {
  powers <- append(powers, power(p_values(i, 100, 0.05)))
}
```
...
for(i in num_subjects) {
  powers <- append(powers, power(p_values(i, 10000, 0.05)))
}
```
