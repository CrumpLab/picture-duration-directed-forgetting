...
# ensure factors are factor class
Switching$subject_means <- Switching$subject_means %>%
  mutate_at(Switching$factors$IVs,factor) %>%
  mutate_at(Switching$factors$subject,factor)

# run ANOVA
Switching$aov.out <- aov(mean_correct ~ encoding_instruction*instruction_switch*test_condition + Error(ID/(encoding_instruction*instruction_switch*test_condition)), Switching$subject_means)

# save printable summaries
Switching$apa_print <- papaja::apa_print(Switching$aov.out)
Switching$apa_print
